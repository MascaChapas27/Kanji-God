...
void KanjiRepository::loadAllKanjis(){
    ...
    for(int i=1;i<=5;i++){
        ...
        #else
        std::wstringstream file(ScrewWindows::kanjisForWindows[i]);
        #endif
        std::wstring data;
        ...
        #else
        std::wstringstream fileStrokes(ScrewWindows::strokesForWindows[i]);
        #endif
...
bool KanjiRepository::checkStroke(Exercise &exercise, sf::VertexArray &stroke, size_t strokeNumber, bool tutorial){
    ...
    float diff = std::abs(realSize-mySize);
