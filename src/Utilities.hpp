...
const float BUTTON_PRESS_SPEED = 5.0;
...
const float INITIAL_PITCH_CORRECT_SOUND = 1.3;

// Pitch increment for the correct answer sound for every correct answer
const float PITCH_INCREMENT_CORRECT_SOUND = 0.1;

// Position for the board that shows the strokes in the tutorial
const float BOARD_TUTORIAL_SHOW_X = WINDOW_WIDTH/3;
const float BOARD_TUTORIAL_SHOW_Y = 6*WINDOW_HEIGHT/20;

// Position for the board that lets the user practice strokes in the tutorial
const float BOARD_TUTORIAL_DRAW_X = BOARD_TUTORIAL_SHOW_X;
const float BOARD_TUTORIAL_DRAW_Y = 14*WINDOW_HEIGHT/20;

// Position for the board used in exercises
const float BOARD_EXERCISE_DRAW_X = BOARD_TUTORIAL_DRAW_X;
const float BOARD_EXERCISE_DRAW_Y = WINDOW_HEIGHT/2;

// When comparing the length of two strokes, the maximun difference allowed
const float MAX_STROKE_SIZE_DIFF = 50;

// For the first and last vertices of a stroke, the maximum distance in pixels
// that is considered correct
const float MAX_STROKE_VERTEX_DIST = 70;

// Thickness for strokes
const float STROKE_THICKNESS = 5;

// Radius for the circle that encloses the first vertex of a strokein a tutorial
const float FIRST_VERTEX_RADIUS = STROKE_THICKNESS*3;
